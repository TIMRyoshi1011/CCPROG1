...
Last modified: Nov. 24, 2024; 9:40PM
Version: 4.2
Acknowledgements: 
...
int main() {
    ...
    for (weekNo = weekNoEdit; weekNo <= 10; weekNo++) { // loops until number of week reaches 10 to keep the program running until week 10
        ...
        if(energonStorage < 800) { //will only execute if the available energon is less than 800
            ...
            return 0; //ends the program entirely
        }  
    }
    
    if (weeklyEnergonStacks > 0) { //will only execute after week 10 and makes sure that this won't be printed when to user inputs 0 on week 10 to avoid blank space
        printf("\n\n");
        printf("---------------------------------------------------------------------------\n\n");
    }
